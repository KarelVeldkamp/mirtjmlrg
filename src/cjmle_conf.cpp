...
arma::vec grad_neg_loglik_A_j_conf_cpp(const arma::vec &response_j, const arma::vec &nonmis_ind_j,
                                  const arma::vec &A_j, const arma::vec &Q_j, const arma::mat &theta, double lambda1 = 0, double lambda2 = 0){
  ...
  // }
  arma::mat grad = -(theta.t() * (nonmis_ind_j % tmp)) % Q_j;
...
arma::mat Update_A_conf_cpp(const arma::mat &A0, const arma::mat &Q, const arma::mat &response,
                       const arma::mat &nonmis_ind, const arma::mat &theta1, double cc, double step_A = 5, double lambda1 = 0, double lambda2 = 0){
  arma::mat A1 = A0.t();
  int J = A0.n_rows;
#pragma omp parallel for num_threads(getmirtjml_threads())
  for(int j=0;j<J;++j){
    double step = step_A;
    arma::vec h = grad_neg_loglik_A_j_conf_cpp(response.col(j), nonmis_ind.col(j), A0.row(j).t(), Q.row(j).t(), theta1, lambda1, lambda2);
...
Rcpp::List Update_A_conf_init_cpp(const arma::mat &A0, const arma::mat &Q, const arma::mat &response,
                            const arma::mat &nonmis_ind, const arma::mat &theta1, double cc, double step_A = 100, double lambda1 = 0, double lambda2=0){
  arma::mat A1 = A0.t();
  int J = A0.n_rows;
...
Rcpp::List cjmle_conf_cpp(const arma::mat &response, const arma::mat &nonmis_ind, arma::mat theta0,
                                    arma::mat A0, arma::mat Q, double cc, double tol, bool print_proc, double lambda1 = 0, double lambda2 = 0){
  int N = theta0.n_rows;
  int J = A0.n_rows;
  int K = theta0.n_cols;
  ...
  Rcpp::List tmp_A = Update_A_conf_init_cpp(A0, Q, response, nonmis_ind, theta1, cc, 100, lambda1, lambda2);
  arma::mat A1 = tmp_A[0];
  ...
  while(eps > tol){
    ...
    A1 = Update_A_conf_cpp(A0, Q, response, nonmis_ind, theta1, cc, A_init_step, lambda1, lambda2);
